...
def content(url,searched=False):
    ...
    try:
        c = client.request(url)
        r = dom_parser2.parse_dom(c, 'div', {'class': re.compile('video-box\sfour-column')})
        r = [(dom_parser2.parse_dom(i, 'a', req='href'), \
            dom_parser2.parse_dom(i, 'div', {'class': 'video-box-title'}), \
            dom_parser2.parse_dom(i, 'span', {'class': 'video-box-duration'}), \
            dom_parser2.parse_dom(i, 'img', req='src')) \
            for i in r if i]
        r = [(urlparse.urljoin(base_domain,i[0][0].attrs['href']), i[1][0].content, i[2][0].content, i[3][0].attrs['src']) for i in r]
